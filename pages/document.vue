  ...
  <div>
    ...
    <div class="databox pb-3">
      <form class="mt-5 py-2 p-3" @submit.prevent="upload()">
        ...
        <hr />
        <h6 class="col-11 mx-auto">
          Total Amount: ₦{{ formatCurrency(total) }}.00
        </h6>

        <button
          class="btn-success shadow w-100 mt-3 btn rounded-pill p-2"
          type="button"
          @click="paid()"
        >
          Order Now
...
export default {
  ...
  methods: {
    ...
    upload() {
      ...
      this.$axios
        .post('/upload-document', this.details, config)
        .then((result) => {
          this.loader = false
